...
2. 填写站点域名、数据库 Root 密码和管理员密码。
3. 首次启动会自动初始化站点并安装 CRM 应用，请耐心等待。
4. 安装完成后，将你的访问域名解析到服务器，并使用该域名访问 `http://站点域名:端口`。
5. 默认管理员账号为 `Administrator`，密码为安装时填写的管理员密码。
6. 数据默认持久化在 `sites`、`db-data`、`redis-data` 目录下。

> 提示：Frappe 根据站点域名路由访问，请使用你填写的站点域名（而非服务器 IP）访问；如需反向代理，请将 `FRAPPE_SITE_NAME` 对应的域名转发到本服务端口。
